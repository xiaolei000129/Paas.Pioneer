...
using Paas.Pioneer.Admin.Core.Domain.Shared.Captcha.Models;
...
namespace Paas.Pioneer.Admin.Core.Domain.Shared.Captcha
{
    ...
    /// 滑块拼图验证
    /// </summary>
    public class SlideJigsawCaptcha : ICaptcha, ITransientDependency
    {
        private readonly RedisAdminKeys _redisAdminKeys;
        public SlideJigsawCaptcha(RedisAdminKeys redisAdminKeys)
        {
        ...
        public async Task<CaptchaOutput> GetAsync()
        {
            ...
            var key = string.Format(_redisAdminKeys.VerifyCodeKey, token);
            await RedisHelper.SetAsync(key, point.X);
        ...
        public async Task<bool> CheckAsync(CaptchaInput input)
        {
            ...
            var key = string.Format(_redisAdminKeys.VerifyCodeKey, input.Token);
            if (await RedisHelper.ExistsAsync(key))
            {
                try
                {
                    var point = JsonConvert.DeserializeObject<PointModel>(input.Data);
                    var x = await RedisHelper.GetAsync<int>(key);
